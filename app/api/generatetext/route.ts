...
async function POST(req: Request){
   try {
     ...
     if(!query && query.lenght < 0 ) throw new Error("Invalid Query")
         ...
         const result : void = await Search_web(query)
...
// this step is for the search api part 
async function Search_web(text:string) {
    if(!text && text.length > 0) throw new Error("Text Issue ( invalid text / text not present )")
        // i am making this fucntion for some refrence , but in reality there willl some another function provided by the google web search api 
        const Search_function_from_the_web = async (text: string) => {
            // example function from he search api 
        }
        const result : void  = await Search_function_from_the_web(text)
